  ...
  try {
    const r = await pool.query(
      `SELECT u.id_usuario, u.nome_usuario, u.email, u.data_criacao, p.nome_perfil
       ...
       WHERE u.id_usuario = $1`, [req.usuario.id]
    );
...
router.put('/me', authRequired, async (req, res) => {
  ...
  try {
    if (email) {
      ...
      if (existe.rows.length) return res.status(409).json({ erro: 'E-mail já utilizado' });
    }
    const fields = [], params = [];
    let idx = 1;
...
router.put('/:id/perfil', authRequired, requirePerfil('Administrador'), async (req, res) => {
  ...
  try {
    await pool.query('UPDATE usuario SET id_perfil_fk=$1 WHERE id_usuario=$2', [id_perfil_fk, req.params.id]);
    res.json({ mensagem: 'Perfil atualizado' });
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ erro: 'Erro interno' });
