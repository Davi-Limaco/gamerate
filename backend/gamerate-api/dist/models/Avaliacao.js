...
async function readAll(filter) {
    ...
    const rows = await db.all(`SELECT a.id_avaliacao, a.titulo, a.texto, a.nota, a.data_publicacao,
     ...
     ${where}
     ORDER BY a.data_publicacao DESC`, params);
    return rows.map(mapAvaliacao);
}
async function readById(id) {
    const db = await Database.connect();
    const avaliacao = await db.get(`SELECT a.*, u.nome_usuario, j.nome_jogo, j.capa, j.id_jogo
     FROM avaliacao a
     JOIN usuario u ON u.id_usuario = a.id_usuario_fk
     JOIN jogo    j ON j.id_jogo    = a.id_jogo_fk
...
async function create({ id_usuario_fk, id_jogo_fk, nota, titulo, texto }) {
    ...
    const { lastID } = await db.run(`INSERT INTO avaliacao (id_usuario_fk, id_jogo_fk, nota, titulo, texto) VALUES (?, ?, ?, ?, ?)`, [id_usuario_fk, id_jogo_fk, notaNum, titulo, texto]);
