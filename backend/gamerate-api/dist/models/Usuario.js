...
async function toggleSeguir(id_seguidor_fk, id_usuario_fk) {
    ...
    SELECT 1 FROM usuario_seguidor WHERE id_seguidor_fk = ? AND id_usuario_fk = ?`, [id_seguidor_fk, id_usuario_fk]);
    if (ja) {
        await db.run(`DELETE FROM usuario_seguidor WHERE id_seguidor_fk = ? AND id_usuario_fk = ?`, [id_seguidor_fk, id_usuario_fk]);
        return false;
    }
    await db.run(`INSERT INTO usuario_seguidor (id_seguidor_fk, id_usuario_fk) VALUES (?, ?)`, [id_seguidor_fk, id_usuario_fk]);
    return true;
}
export default {
