...
async function create({ nome_jogo, desenvolvedora, data_lancamento, descricao, capa, generos = [], plataformas = [], nota_media = null, total_avaliacoes = 0, }) {
    ...
    if (nome_jogo && desenvolvedora && data_lancamento && descricao) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [nome_jogo, desenvolvedora, data_lancamento, descricao, nota_media, total_avaliacoes, capa || null]);
...
async function update({ id, nome_jogo, desenvolvedora, data_lancamento, descricao, capa }) {
    const db = await Database.connect();
    const { changes } = await db.run(`UPDATE jogo SET nome_jogo = ?, desenvolvedora = ?, data_lancamento = ?, descricao = ?, capa = ?
     WHERE id_jogo = ?`, [nome_jogo, desenvolvedora, data_lancamento, descricao, capa, id]);
    if (changes === 1)
        return readById(id);
    throw new Error('Jogo não encontrado');
}
