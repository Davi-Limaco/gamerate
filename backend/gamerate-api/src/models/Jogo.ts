/**
 * models/Jogo.ts — Model para a entidade Jogo
 */

import Database from '../database/database.js';
import type { Jogo, Genero, Plataforma } from '../types/index.js';
import { badRequest, notFound } from '../errors/HttpError.js';

interface JogoCreateInput {
  nome_jogo: string;
...
async function readAll(filter?: JogoFilter): Promise<Jogo[]> {
  ...
  return await db.all<Jogo>(sql, params);
}

async function readById(id: number): Promise<Jogo> {
  const db = await Database.connect();

  const jogo = await db.get<Jogo>(`SELECT * FROM jogo WHERE id_jogo = ?`, [id]);

  if (!jogo) throw notFound('Jogo não encontrado');

  const generos = await db.all<Genero>(
  ...
  const plataformas = await db.all<Plataforma>(
...
async function create(input: JogoCreateInput): Promise<Jogo> {
  const db = await Database.connect();
  const { nome_jogo, desenvolvedora, data_lancamento, descricao, capa, generos = [], plataformas = [] } = input;

  if (!nome_jogo || !desenvolvedora || !data_lancamento || !descricao) {
    throw badRequest('Campos obrigatórios: nome_jogo, desenvolvedora, data_lancamento, descricao');
  }
...
async function update(input: JogoUpdateInput): Promise<Jogo> {
  const db = await Database.connect();
  const { id, nome_jogo, desenvolvedora, data_lancamento, descricao, capa } = input;
...
async function atualizarNota(id: number): Promise<void> {
  ...
  const nota = await db.get<{ media: number | null; total: number }>(
  ...
  await db.run(
    ...
    [nota?.media || null, nota?.total || 0, id]
  );
}
...
async function getStats(): Promise<Stats> {
  ...
  const total_jogos = await db.get<{ count: number }>(`SELECT COUNT(*) AS count FROM jogo`);
  const total_aval = await db.get<{ count: number }>(`SELECT COUNT(*) AS count FROM avaliacao`);
  const total_usuarios = await db.get<{ count: number }>(`SELECT COUNT(*) AS count FROM usuario`);
  const total_plat = await db.get<{ count: number }>(`SELECT COUNT(*) AS count FROM plataforma`);
...
async function getDestaques(): Promise<Destaques> {
  const db = await Database.connect();

  const lancamentos = await db.all<Jogo>(`
  ...
  const melhores = await db.all<Jogo>(`
    SELECT id_jogo, nome_jogo, desenvolvedora, data_lancamento, 
           nota_media, total_avaliacoes, capa
    FROM jogo
