...
async function up() {
  ...
  try {
    for (const perfil    of seed.perfis)     await Perfil.create(perfil);
    for (const usuario   of seed.usuarios)   await Usuario.create(usuario);
    for (const plataforma of seed.plataformas) await Plataforma.create(plataforma);
    for (const genero    of seed.generos)    await Genero.create(genero);
